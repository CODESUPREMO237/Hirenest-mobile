...
class MySalesPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('My Sales'), elevation: 0),
      body: salesAsync.when(
        data: (orders) {
          if (orders.isEmpty) return _buildEmptyState(context);
          return RefreshIndicator(
            onRefresh: () async => ref.invalidate(mySalesProvider),
        ...
        error: (error, stack) => Center(
          child: Column(
            ...
            children: [
              Text('Error loading sales', style: TextStyle(color: AppColors.error)),
              const SizedBox(height: AppSpacing.sm),
              ElevatedButton(onPressed: () => ref.invalidate(mySalesProvider), child: const Text('Retry')),
            ],
          ),
        ),
      ),
  ...
  Widget _buildEmptyState(BuildContext context) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(Icons.storefront_outlined, size: 64, color: AppColors.textMutedLight),
          const SizedBox(height: AppSpacing.lg),
          Text('No sales yet', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
          const SizedBox(height: AppSpacing.sm),
          Text('When someone buys your products, they will appear here.', style: TextStyle(color: AppColors.textSecondaryLight)),
        ],
      ),
    );
  }
          ...
          child: Column(
            ...
            children: [
              ...
              if (order.status == 'PAID_ESCROW') ...[
                const SizedBox(height: AppSpacing.lg),
                Text('Payment secured! Ship the item to the buyer.', style: TextStyle(fontSize: 12, color: AppColors.warning, fontWeight: FontWeight.w600)),
              ] else if (order.status == 'SHIPPED') ...[
                const SizedBox(height: AppSpacing.lg),
                Text('Awaiting delivery confirmation from buyer.', style: TextStyle(fontSize: 12, color: AppColors.textSecondaryLight)),
              ],
            ],
          ),
        ),
  ...
  Widget _buildStatusBadge(String status) {
    ...
    switch (status) {
      ...
      case 'PAID_ESCROW': color = AppColors.info; label = 'Ship Item'; break;
