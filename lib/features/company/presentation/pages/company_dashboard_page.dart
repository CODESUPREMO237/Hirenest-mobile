    ...
    return Scaffold(
      ...
      body: companyAsync.when(
        data: (company) => RefreshIndicator(
          // FIX: Improved refresh logic
          onRefresh: () async {
            return await ref.refresh(myCompanyProvider.future);
          },
          child: SingleChildScrollView(
            // FIX: Force scrollability to allow pull-to-refresh on short screens
      ...
      floatingActionButton: companyAsync.hasValue
          ...
          children: [
            Expanded(
              child: _ActionButton(
                ...
                label: 'Admins',
                color: Colors.purple,
                onPressed: () => context.push('/company/admins'),
              ),
            ),
            const SizedBox(width: 12),
            Expanded(
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(company.industry ?? 'General', style: TextStyle(color: Colors.grey[600])),
                    ],
                  ),
                ),
              ],
  ...
  Widget build(BuildContext context) {
    return Card(
      child: Padding(
        ...
        child: Column(
          children: [
            _InfoRow(icon: Icons.description, label: 'About', value: company.description ?? '...'),
            const Divider(),
            _InfoRow(icon: Icons.language, label: 'Website', value: company.website ?? 'N/A'),
            const Divider(),
            _InfoRow(icon: Icons.email, label: 'Contact', value: company.email ?? company.email ?? 'N/A'),
          ],
        ),
      ),
    );
