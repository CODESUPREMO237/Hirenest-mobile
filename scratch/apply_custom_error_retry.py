...
def extract_retry(old_widget):
    # look for ref.refresh(...) or ref.invalidate(...) or just something assigned to onPressed
    match = re.search(r"(ref\.(refresh|invalidate)\([^)]+\))", old_widget)
    if match:
        return match.group(1)
    
    # or if there's an onPressed: () => function()
    match2 = re.search(r"onPressed:\s*(\(\)\s*=>\s*[^,}]+)", old_widget)
    if match2:
        ...
        if content[start_idx:start_idx+6] == "Center" or \
           ...
           content[start_idx:start_idx+6] == "Column":
            ...
            if paren_start != -1:
                ...
                if end_paren != -1:
                    old_widget = content[start_idx:end_paren+1]
                    retry_action = extract_retry(old_widget)
                    
                    is_sliver = "SliverFillRemaining" in content[start_idx:paren_start]
                    
                    retry_str = ""
                    if retry_action:
                        if retry_action.startswith("()"):
                            retry_str = f", onRetry: {retry_action}"
                        else:
                            retry_str = f", onRetry: () => {retry_action}"
                    
                    if is_sliver:
                        replacement = f"SliverFillRemaining(child: CustomErrorWidget(error: {var_err}{retry_str}))"
                    else:
                        replacement = f"CustomErrorWidget(error: {var_err}{retry_str})"
...
for root, _, files in os.walk(lib_dir):
    for file in files:
        if file.endswith(".dart"):
            ...
            # Change message: error.toString() to error: error
